...
const roboticsCategories = [
  {
    ...
    images: ['/images/kit1.png', '/images/kit2.png', '/images/kit1.png']
  },
  {
    id: 'escolar',
...
const Robotics = () => {
  ...
  // Spotlight Cyan/Verde Eléctrico
  ...
  const openWhatsapp = (topic) => {
    window.open(`https://wa.me/51987564941?text=Hola InnovaLab, info sobre: ${topic}`, '_blank');
  };
  ...
  return (
    <div className="font-sans bg-gray-50 min-h-screen relative overflow-x-hidden selection:bg-emerald-500 selection:text-white">
      ...
      <style>{`
        ...
        /* Efecto Glitch sutil para texto */
        @keyframes glitch {
          0% { text-shadow: 2px 2px 0px #0ea5e9, -2px -2px 0px #10b981; }
          2% { text-shadow: -2px 2px 0px #0ea5e9, 2px -2px 0px #10b981; }
          4% { text-shadow: 2px -2px 0px #0ea5e9, -2px 2px 0px #10b981; }
          6% { text-shadow: none; }
      ...
      {/* --- 1. HERO SECTION (DISEÑO "NEURAL RADAR") --- */}
      <section 
        ref={heroRef}
        onMouseMove={handleMouseMove}
        className="relative min-h-screen flex flex-col justify-center items-center bg-[#020617] overflow-hidden pt-20 group"
      >
        {/* FONDO DOTS */}
        ...
        {/* RADAR / SCANNER GIRATORIO (Elemento Visual Único) */}
        ...
        {/* Nodos flotantes (simulando objetivos detectados) */}
        ...
        <div className="relative z-20 text-center px-6 max-w-6xl mx-auto">
          ...
          >
             ...
             <div className="px-4 py-1.5 rounded border border-emerald-500/30 bg-[#020617]/80 text-emerald-400 text-xs font-mono font-bold tracking-widest uppercase flex items-center gap-2 backdrop-blur-md">
          ...
          >
             <h1 className="text-6xl md:text-9xl font-black text-white leading-none tracking-tight mb-4 glitch-text cursor-default">
               <span className="text-emerald-500 opacity-50 text-4xl md:text-7xl align-top mr-2">[</span>
               ROBÓTICA
               <span className="text-emerald-500 opacity-50 text-4xl md:text-7xl align-top ml-2">]</span>
             </h1>
             
             <div className="flex items-center justify-center gap-4 mt-6">
                <span className="h-[1px] w-12 bg-gradient-to-r from-transparent to-cyan-500"></span>
                <p className="text-cyan-400 font-mono text-sm md:text-lg tracking-[0.2em] uppercase">
                  Automación • IoT • Inteligencia Artificial
                </p>
          ...
          <motion.p 
            ...
            className="mt-10 text-lg md:text-xl text-gray-400 max-w-2xl mx-auto leading-relaxed font-light"
          >
            Donde el hardware se encuentra con la inteligencia. Desarrollamos la tecnología del futuro hoy.
          </motion.p>
          ...
          >
             <button onClick={() => openWhatsapp('Cotización Robótica')} className="group px-8 py-4 bg-emerald-600 text-white font-bold text-lg rounded-full hover:bg-emerald-500 transition-all flex items-center gap-3 shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_35px_rgba(16,185,129,0.6)]">
                <Terminal size={20} /> Iniciar Proyecto
             </button>
             <a href="#galeria" className="group px-8 py-4 border border-gray-700 text-gray-300 font-medium text-lg rounded-full hover:border-emerald-500 hover:text-emerald-400 transition-all backdrop-blur-sm flex items-center gap-2">
                <Settings size={20} className="group-hover:rotate-90 transition-transform duration-700"/> Ver Kits
             </a>
          </motion.div>
        ...
        <motion.div 
          ...
          className="absolute bottom-10 left-1/2 -translate-x-1/2 text-emerald-500/50 flex flex-col items-center gap-2"
        >
           <ChevronDown size={24} />
        </motion.div>
      </section>
      ...
      {/* --- 3. GALERÍA SCROLLABLE (Split) --- */}
      <section id="galeria" className="py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-brand-dark mb-4">Nuestras Áreas</h2>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 sticky top-24 z-10">
                  ...
                  <button 
                    ...
                    className={`w-full text-left px-5 py-4 rounded-xl transition-all duration-200 flex items-center justify-between group ${
                      activeCategory.id === cat.id 
                        ? 'bg-[#020617] text-white shadow-md border-l-4 border-emerald-500' 
                        : 'hover:bg-gray-100 text-gray-600'
                    }`}
                  >
                    <span className="font-bold">{cat.title}</span>
                    {activeCategory.id === cat.id && <Bot size={18} className="text-emerald-400" />}
                  </button>
                ))}
              </div>
              <div className="mt-6 bg-emerald-50/80 p-6 rounded-2xl border border-emerald-100 backdrop-blur-md">
                ...
                <button onClick={() => openWhatsapp('Asesoría Tesis')} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg text-sm transition-colors shadow-lg shadow-emerald-200">
                  Consultar Ahora
                </button>
              </div>
            </div>
            ...
            <div className="lg:col-span-8">
              <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100 h-[700px] flex flex-col">
                  <div className="mb-6 border-b border-gray-100 pb-6 shrink-0 flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
                      <div>
                          <h3 className="text-2xl font-bold text-gray-900 mb-1">{activeCategory.title}</h3>
                          <p className="text-gray-500 text-sm">{activeCategory.desc}</p>
                      </div>
                      <div className="flex gap-2 flex-wrap">
                        {activeCategory.features.map((feat, idx) => (
                          <span key={idx} className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1">
                            <Check size={10}/> {feat}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="overflow-y-auto custom-scrollbar pr-2 flex-grow">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pb-4">
                        ...
                        <div key={idx} className="relative group rounded-xl overflow-hidden cursor-pointer h-40 md:h-48 bg-gray-100 border border-gray-200" onClick={() => setSelectedImage(imgSrc)}>
                            <img src={imgSrc} alt="Proyecto" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onError={(e) => {e.target.src = 'https://via.placeholder.com/400x400?text=Proyecto'}} />
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center relative z-10">
            <div>
              ...
              <p className="text-gray-400 text-lg mb-8 leading-relaxed">
                Descarga el brochure completo con la lista de componentes, precios de kits educativos y temario de cursos.
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                <a href="/portafolio_robotica.pdf" download="Catalogo_Robotica_InnovaLab" className="flex items-center justify-center gap-3 bg-white text-brand-dark font-bold py-4 px-8 rounded-xl hover:bg-gray-100 transition-all shadow-lg hover:-translate-y-1">
                  <Download size={22} className="text-emerald-600" /> Descargar PDF
                </a>
                <a href="/portafolio_robotica.pdf" target="_blank" className="flex items-center justify-center gap-3 border border-gray-600 text-gray-300 font-bold py-4 px-8 rounded-xl hover:border-white hover:text-white transition-all">
            ...
            <div className="relative group">
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-2 shadow-2xl border border-gray-700 transform group-hover:scale-[1.02] transition-transform duration-500">
                ...
                <div className="bg-white h-[400px] w-full rounded-b-xl overflow-hidden relative">
                   <iframe src="/portafolio_robotica.pdf#toolbar=0" title="Visor Portafolio" className="w-full h-full">
                   ...
                   <a href="/portafolio_robotica.pdf" target="_blank" className="absolute bottom-4 right-4 bg-black/80 text-white p-2 rounded-lg hover:bg-emerald-500 transition-colors backdrop-blur-sm" title="Ver pantalla completa"><Maximize2 size={20} /></a>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-[#020617] text-center relative overflow-hidden border-t border-gray-800">
         ...
         <div className="relative z-10 max-w-3xl mx-auto px-6">
            ...
            <p className="text-gray-400 text-lg mb-10">
               Desde un robot seguidor de línea hasta un sistema de automatización industrial. Lo hacemos realidad.
            </p>
            <a href="https://wa.me/51987564941" target="_blank" className="inline-flex items-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 px-10 rounded-full shadow-lg shadow-emerald-500/20 hover:scale-105 transition-transform">
              <MessageCircle size={24} /> Contactar Ingeniero
            </a>
         </div>
      </section>
        ...
        {selectedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 backdrop-blur-md" onClick={() => setSelectedImage(null)}>
            <div className="relative max-w-5xl w-full flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
              ...
              <img src={selectedImage} alt="Zoom" className="max-h-[85vh] rounded shadow-2xl border border-white/10" onError={(e) => {e.target.src = 'https://via.placeholder.com/800x600?text=Imagen+No+Encontrada'}} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
