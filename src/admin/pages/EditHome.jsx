...
import { Image as ImageIcon, Loader, Trash2, Plus, Save, LayoutGrid, Layers } from 'lucide-react';

const EditHome = () => {
  const [loading, setLoading] = useState(true);
  const [uploading, setUploading] = useState(false);

  // --- ESTADO 1: IMÁGENES DE SERVICIOS (Las 3 de arriba) ---
  ...
  // --- ESTADO 2: GALERÍA (Las Cyber Cards de abajo) ---
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Cargar Imágenes de Servicios (Documento único)
        const docRef = doc(db, "web_content", "home_services");
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) setServiceImages(docSnap.data());

        // 2. Cargar Galería (Colección)
  ...
  const handleServiceUpload = async (e, key) => {
    ...
    if (url) {
      ...
      await setDoc(doc(db, "web_content", "home_services"), updated); // Guardado automático
    }
    setUploading(false);
  };
  ...
  const handleAddCard = async (e) => {
    ...
    setNewCard({ title: '', subtitle: '', tag: '', image: '' }); // Reset
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      ...
      {/* SECCIÓN 1: IMÁGENES DE SERVICIOS */}
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
          <Layers className="text-indigo-600"/> Imágenes de Servicios (Ecosistema)
        </h2>
      ...
      {/* SECCIÓN 2: GALERÍA DE PROYECTOS (CYBER CARDS) */}
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
          <LayoutGrid className="text-indigo-600"/> Galería de Resultados (Portafolio)
        </h2>

        {/* Formulario Agregar */}
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 mb-8">
          <h3 className="text-sm font-bold text-gray-600 mb-4">Agregar Nueva Tarjeta</h3>
          <div className="grid md:grid-cols-4 gap-4 items-end">
            <div className="md:col-span-1">
                <label className="block text-xs font-bold text-gray-400 mb-1">Imagen</label>
                <label className="cursor-pointer flex items-center justify-center h-12 bg-white border border-gray-300 rounded-lg text-xs text-gray-500 hover:border-indigo-500 hover:text-indigo-500 transition-colors">
                    {uploading ? <Loader className="animate-spin"/> : (newCard.image ? "¡Imagen cargada!" : <><ImageIcon size={16} className="mr-2"/> Subir Foto</>)}
                    <input type="file" className="hidden" onChange={handleGalleryUpload} disabled={uploading}/>
                </label>
            </div>
            <div className="md:col-span-1">
                <input className="w-full p-3 border rounded-lg text-sm" placeholder="Título (ej: Prototipo)" value={newCard.title} onChange={e => setNewCard({...newCard, title: e.target.value})}/>
            </div>
            <div className="md:col-span-1">
                <input className="w-full p-3 border rounded-lg text-sm" placeholder="Subtítulo (ej: Alta Fidelidad)" value={newCard.subtitle} onChange={e => setNewCard({...newCard, subtitle: e.target.value})}/>
            </div>
            <div className="md:col-span-1 flex gap-2">
                <input className="w-full p-3 border rounded-lg text-sm" placeholder="Tag (ej: 3D)" value={newCard.tag} onChange={e => setNewCard({...newCard, tag: e.target.value})}/>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {galleryItems.length === 0 && <p className="col-span-4 text-center text-gray-400 py-10">No hay tarjetas en la galería.</p>}
        </div>
      </section>
